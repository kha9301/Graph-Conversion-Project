...
public class DoublyLinkedList<AnyType> implements List<AnyType> {
	...
	/*
	 * public void displayData() { int i = -1; Node current = header; while
	 * (current != null) { // if current == null, that means the pointer // has
	 * reached to the very last of the list if (i == -1) { System.out.println(
	 * "HEADER : " + current.getData()); } else if (i == theSize) {
	 * System.out.println("TRAILER : " + current.getData()); } else {
	 * System.out.println(i + " : " + current.getData()); } current =
	 * current.getNext(); i++; } }
	 */
